...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $stmt = $pdo->prepare("INSERT INTO student_profile 
    (name, student_number, birth_date, age, gender, civil_status, nationality, religion, contact_number, email, 
    home_address, current_address, college, program, year_level, section, student_status, scholarship, academic_standing, 
    parent_name, parent_relationship, parent_contact, parent_address, parent_occupation, emergency_contact_name, 
    emergency_contact_number, medical_conditions, disabilities, medications, mental_health_history, personal_physician, 
    stress_experience, coping_mechanism, past_counseling, support_system, comfort_seeking_help, counseling_type, 
    preferred_mode, preferred_counselor, availability, hobbies, organizations, leadership_roles, volunteer_work, 
    consent, agreement) 
    VALUES 
    (:name, :student_number, :birth_date, :age, :gender, :civil_status, :nationality, :religion, :contact_number, :email, 
    :home_address, :current_address, :college, :program, :year_level, :section, :student_status, :scholarship, :academic_standing, 
    ...
    $stmt->execute([
        ...
        ':mental_health_history' => $_POST['mental_health_history'] ?? '',
        ':personal_physician' => $_POST['personal_physician'] ?? '',
        ':stress_experience' => $_POST['stress_experience'] ?? '',
        ':coping_mechanism' => $_POST['coping_mechanism'] ?? '',
        ':past_counseling' => $_POST['past_counseling'] ?? '',
        ':support_system' => $_POST['support_system'] ?? '',
        ':comfort_seeking_help' => $_POST['comfort_seeking_help'] ?? '',
        ':counseling_type' => isset($_POST['counseling_type']) ? implode(',', $_POST['counseling_type']) : '',
        ':preferred_mode' => $_POST['preferred_mode'] ?? '',
        ':preferred_counselor' => $_POST['preferred_counselor'] ?? '',
        ':availability' => $_POST['availability'] ?? '',
        ':hobbies' => $_POST['hobbies'] ?? '',
        ':organizations' => $_POST['organizations'] ?? '',
        ':leadership_roles' => $_POST['leadership_roles'] ?? '',
        ':volunteer_work' => $_POST['volunteer_work'] ?? '',
        ':consent' => $_POST['consent'] ?? 'No',
        ':agreement' => $_POST['agreement'] ?? 'No'
    ]);
