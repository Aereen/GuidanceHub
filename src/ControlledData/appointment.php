...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_appointment'])) {
    try {
        ...
        $sql = "INSERT INTO appointments (ticket_id, name, email, college, year_level, section, feelings, need_counselor, counseling_type, first_date, first_time, second_date, second_time)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->execute();
        $stmt->close(); // Close the statement to avoid the "Commands out of sync" error

        echo "<script>alert('Appointment scheduled successfully! Log In to check your appointment status.'); window.location.href='/src/ControlledData/appointment.php';</script>";
    } catch (Exception $e) {
